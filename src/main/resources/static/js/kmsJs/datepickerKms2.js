	...
	(function($) {
		var
			...
			get_today_full = function() {
				return get_today('d')+'/'+get_today('m')+'/'+get_today('y');
			},
			get_current_full = function() {
				return get_current('d')+'/'+get_current('m')+'/'+get_current('y');
			},
			...
			return $(this).each(function(){
				if($(this).is('input')&&!$(this).hasClass('picker-input')) {
					...
					var
						...
						picker_lang = (input.data('lang')&&(input.data('lang') in i18n)) ? input.data('lang') : 'ko',
