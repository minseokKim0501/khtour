...
(function() {
	...
	function parseDate(str, options) {
		...
		if ((matches = str.match(/^(\d{4})\s*\-\s*([01]\d)\s*\-\s*([0-3]\d)$/))
			...
			return input_date.isValid() ? input_date : false;
				...
				.match(/^([01]?\d)\s*[\/\-\.]?\s*([0-3]?\d)\s*[\/\-\.]?\s*((\d{2})?\d{2})$/))
				&& parseInt(matches[1]) > 0
				&& parseInt(matches[1]) < 13
				&& parseInt(matches[2]) > 0 && parseInt(matches[2]) < 32) {
			if (matches[3].length == 2) {
				matches[3] = Math.floor(new Date().getFullYear() / 100) * 100
						- ((parseInt(matches[3]) >= 50) ? 100 : 0)
						+ parseInt(matches[3]);
			}
			input_date = dayjs(matches[3] + '-'
					+ ((matches[1].length > 1) ? '' : '0') + matches[1] + '-'
					+ ((matches[2].length > 1) ? '' : '0') + matches[2]);
			if (input_date.isValid()) {
				if (options
						&& ((options.minDate && input_date.isBefore(
								options.minDate, 'date')) || (options.maxDate && input_date
								.isAfter(options.maxDate, 'date')))) {
					return false;
				}
				return input_date;
			}
	...
	function updateYearPicker($input) {
		...
		var today_disabled = ((options.minDate && today.isBefore(
				options.minDate, 'date'))
				|| (options.maxDate && today.isAfter(options.maxDate, 'date')) || (today
				.year() >= startYear && today.year() <= endYear));

		// Build html
		var html = '<div class="d-flex justify-content-between align-items-center datepicker-btns">'
				+ '<div class="font-weight-bold"><button type="button" id="'
				+ input_id
				+ '-picker-btn" class="btn font-weight-bold dropdown-toggle">'
				+ startDate.year()
				+ ' - '
				+ endDate.year()
				+ '</button></div><div>'
				+ '<a id="'
				+ input_id
				+ '-picker-prev-link" class="btn btn-link px-1 mx-0'
				+ ((!options.minDate || options.minDate.isBefore(startDate,
						'date')) ? '' : ' disabled')
				+ '" href="javascript:void(0)" title="Go to Previous Years"><i class="fas fa-chevron-left fa-fw"></i></a>'
				+ '<a id="'
				+ input_id
				+ '-picker-today-link" class="btn btn-link px-1 mx-0'
				+ (today_disabled ? ' disabled' : '')
				+ '" href="javascript:void(0)" title="Go to Current Year"><i class="far fa-calendar-check fa-fw"></i></a>'
				+ '<a id="'
				+ input_id
				+ '-picker-next-link" class="btn btn-link px-1 mx-0'
				+ ((!options.maxDate || options.maxDate
						.isAfter(endDate, 'date')) ? '' : ' disabled')
				+ '" href="javascript:void(0)" title="Go to Next Years"><i class="fas fa-chevron-right fa-fw"></i></a>'
				+ '</div></div>'
		...
		for (var i = 0; i < (endYear - startYear + 1); i++) {
			...
			var disabled = ((options.minDate && options.minDate.isAfter(i_date
					.endOf('year'), 'date')) || (options.maxDate && options.maxDate
					.isBefore(i_date.startOf('year'), 'date')));
			html += ((i % 5 == 0) ? '<tr>' : '')
					...
					+ ((input_date && input_date.isSame(i_date, 'year')) ? 'active btn-info'
							: 'btn-outline-dark border-white'
									+ (i_date.isSame(today, 'year') ? ' today'
											: ''))
					+ '" '
					+ (disabled ? 'disabled="disabled"' : 'data-year="'
							+ i_date.year() + '"') + '>' + i_date.year()
					+ '</button></td>' + ((i % 5 == 4) ? '</tr>' : '');
		}
		html += '</table>';
		jQuery('#' + input_id + '-picker-content').html(html).find(
	...
	function updateMonthPicker($input) {
		...
		var html = '<div class="d-flex justify-content-between align-items-center datepicker-btns">'
				...
				+ input_id
				+ '-picker-prev-link" class="btn btn-link px-1 mx-0'
				+ ((!options.minDate || options.minDate.isBefore(viewDate
						.startOf('year'), 'date')) ? '' : ' disabled')
				+ '" href="javascript:void(0)" title="Go to Previous Year" data-unit="month"><i class="fas fa-chevron-left fa-fw"></i></a>'
				+ '<a id="'
				+ input_id
				+ '-picker-today-link" class="btn btn-link px-1 mx-0'
				+ (today_disabled ? ' disabled' : ' ')
				+ '" href="javascript:void(0)" title="Go to Current Month"><i class="far fa-calendar-check fa-fw"></i></a>'
				+ '<a id="'
				+ input_id
				+ '-picker-next-link" class="btn btn-link px-1 mx-0'
				+ ((!options.maxDate || options.maxDate.isAfter(viewDate
						.endOf('year'), 'date')) ? '' : ' disabled')
				+ '" href="javascript:void(0)" title="Go to Next Year" data-unit="month"><i class="fas fa-chevron-right fa-fw"></i></a>'
				+ '</div></div>'
		...
		for (var i = 0; i < 12; i++) {
			...
			var disabled = ((options.minDate && options.minDate.isAfter(i_date
					.endOf('month'), 'date')) || (options.maxDate && options.maxDate
					.isBefore(i_date.startOf('month'), 'date')));
			html += ((i % 3 == 0) ? '<tr>' : '')
					+ '<td class="text-center p-0"><button type="button" class="btn btn-block px-0 '
					+ ((input_date && input_date.isSame(i_date, 'month')) ? 'active btn-info'
							: 'btn-outline-dark border-white'
									+ (i_date.isSame(today, 'month') ? ' today'
											: ''))
					+ '" '
					+ (disabled ? 'disabled="disabled"' : 'data-date="'
							+ i_date.format('YYYY-MM-DD"')) + '>'
					+ i_date.format('MMM') + '</button></td>'
					+ ((i % 3 == 2) ? '</tr>' : '');
		}
		for (var i = 0; i < 6; i++) {
			html += ((i % 3 == 0) ? '<tr>' : '')
	...
	function updateDatePicker($input) {
		...
		var today_disabled = ((options.minDate && today.isBefore(
				options.minDate, 'date'))
				|| (options.maxDate && today.isAfter(options.maxDate, 'date')) || today
				.isSame(viewDate, 'month'));

		// Build html
		var html = '<div class="d-flex justify-content-between align-items-center datepicker-btns">'
				+ '<div class="font-weight-bold"><button type="button" id="'
				+ input_id
				+ '-picker-btn" class="btn font-weight-bold dropdown-toggle px-2">'
				+ viewDate.format('MMMM YYYY')
				+ '</button></div><div class="text-nowrap">'
				+ '<a id="'
				+ input_id
				+ '-picker-prev-link" class="btn btn-link px-1 mx-0'
				+ ((!options.minDate || options.minDate.isBefore(viewDate
						.startOf('month'), 'date')) ? '' : ' disabled')
				+ '" href="javascript:void(0)" title="Go to Previous Month" data-unit="month"><i class="fas fa-chevron-left fa-fw"></i></a>'
				+ '<a id="'
				+ input_id
				+ '-picker-today-link" class="btn btn-link px-1 mx-0'
				+ (today_disabled ? ' disabled' : '')
				+ '" href="javascript:void(0)" title="Go to Today"><i class="far fa-calendar-check fa-fw"></i></a>'
				+ '<a id="'
				+ input_id
				+ '-picker-next-link" class="btn btn-link px-1 mx-0'
				+ ((!options.maxDate || options.maxDate.isAfter(viewDate
						.endOf('month'), 'date')) ? '' : ' disabled')
				+ '" href="javascript:void(0)" title="Go To Next Month" data-unit="month"><i class="fas fa-chevron-right fa-fw"></i></a>'
				+ '</div></div>'
				+ '<table class="table table-sm table-borderless text-center datepicker-table mb-1"><thead class="thead-light"><tr>';
		// ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].forEach(function
		// (day) {
		// html += '<th class="py-1" scope="col" style="width:14.28%">' + day +
		// '</th>';
		// });
		for (var i = 0; i < 7; i++) {
			html += '<th class="py-1" scope="col" style="width:14.28%">'
					+ viewDate.day(i).format('dd') + '</th>';
		}
		...
		for (var i = startMonth.day(); i > 0; i--) {
			...
			var disabled = ((options.minDate && options.minDate.isAfter(i_date
					.endOf('date'), 'date')) || (options.maxDate && options.maxDate
					.isBefore(i_date.startOf('date'), 'date')));
			html += '<td class="text-center p-0"><button type="button" class="btn btn-block px-0 '
					+ ((input_date && input_date.isSame(i_date, 'date')) ? 'active btn-info'
							: 'btn-outline-secondary border-white'
									+ (i_date.isSame(today, 'date') ? ' today'
											: ''))
					+ '" '
					+ (disabled ? 'disabled="disabled"' : 'data-date="'
							+ i_date.format('YYYY-MM-DD"'))
					+ '>'
					+ i_date.format('D') + '</button></td>';
			dow++;
		}
		var rows = 0;
		...
		if (dow > 0) {
			...
			for (var i = 1; i <= (7 - dow); i++) {
				...
				var disabled = ((options.minDate && options.minDate.isAfter(
						i_date.endOf('date'), 'date')) || (options.maxDate && options.maxDate
						.isBefore(i_date.startOf('date'), 'date')));
				html += '<td class="text-center p-0"><button type="button" class="btn btn-block px-0 '
						+ ((input_date && input_date.isSame(i_date, 'date')) ? 'active btn-info'
								: ' btn-outline-secondary border-white'
		...
		jQuery('#' + input_id + '-picker-content').html(html).find(
				...
				function() {
					...
					$input.val(newDate.format(options.format)).popover('hide')
							.trigger('change');
				});
		jQuery('#' + input_id + '-picker-prev-link').on('click', function() {
	...
	jQuery.fn.datepicker = function(options) {
		...
		var bs_version = parseInt(jQuery.fn.dropdown.Constructor.VERSION
				.replace(/\..+$/, ''));
		if (bs_version < 4) {
			console.error('Invalid bootstrap version ' + bs_version
					+ ' detected');
		}
		...
		if (typeof options == 'string') {
			...
			case 'date':
				if (arguments.length > 1) {
					var newDate = (arguments[1]) ? parseDate(arguments[1],
							input_options) : null;
					return this.val((newDate && newDate.isValid()) ? newDate
							.format(input_options.format) : '');
				} else {
		...
		// Convert to date type if screen doesn't meet the mininum width or an
		// IOS device
		if ((common_options.minScreenWidth && window.screen.width < common_options.minScreenWidth)
			...
			return this.each(function() {
				...
				this.type = 'date';
				jQuery(
						'[data-toggle="datepicker"][data-target="#' + this.id
								+ '"]').add(
						$input.siblings().find('[data-toggle="datepicker"]'))
						.on('click', function() {
							$input.focus();
						});
			});
		...
		return this
				.each(function() {
					...
					var minDate = $input.attr('min') || $input.data('mindate')
							|| common_options.minDate;
					if (minDate && (minDate = dayjs(minDate))
							&& minDate.isValid()) {
						input_options.minDate = minDate.startOf('date');
					}
					var maxDate = $input.attr('max') || $input.data('maxdate')
							|| common_options.maxDate;
					if (maxDate && (maxDate = dayjs(maxDate))
							&& maxDate.isValid()) {
						input_options.maxDate = maxDate.endOf('date');
					}
					$input.data('datepicker-options', input_options);
					var input_id = this.id;
					var $toggles = $input.siblings().find(
							'[data-toggle="datepicker"]:not([data-target])');
					if (this.id) {
						$toggles = $toggles
								.add('[data-toggle="datepicker"][data-target="#'
										+ this.id + '"]');
					} else {
						input_id = 'input-'
							...
							.on(
									...
									function() {
										this.value = this.value.replace(
												/^\s+|\s+$/g, '');
										var input_options = $input
												.data('datepicker-options');
										var newDate = parseDate(this.value,
												input_options);
										var newValue = (newDate
												&& newDate.isValid()
												&& !(input_options.minDate && newDate
														.isBefore(input_options.minDate)) && !(input_options.maxDate && newDate
												.isAfter(input_options.maxDate))) ? newDate
												.format(input_options.format)
												: '';
							...
							.popover(
									{
										...
										placement : 'bottom',
										sanitize : false,
										title : '<button type="button" class="close mt-n1" data-dismiss="popover">&times;</button>'
												+ (($label.length > 0) ? $label
														.html() : 'Date'),
										template : '<div id="'
												+ input_id
												+ '-picker-popover" class="popover datepicker-popover bs-popover-bottom" role="tooltip" style="width:'
												+ input_options.popoverWidth
												+ ';"><div class="arrow"></div><div id="'
												+ input_id
												+ '-popover-body" class="popover-body border-bottom"></div><div class="popover-footer bg-light text-right px-3 py-2 rounded-lg" hidden="hidden"><button type="button" class="btn btn-secondary btn-sm" title="Close the picker" data-dismiss="popover"><i class="fas fa-times"></i> Close</button></div></div>',
										trigger : (($toggles.length > 0) ? 'manual'
												: 'click'),
										popperConfig : {
										/*
										 * modifiers: { hide: { enabled: false },
